...
public:
    ...
    virtual unsigned int GetValue(unsigned int index) = 0;

    // 这是自清理方法
    // 该方法调用后，这个接口指针就失效了
    virtual void Cleanup() = 0;

    enum class FibonacciType {
        Standard, // 第一项是 0， 第二项是 1
        Extend // 前两项都是 1
    };
};
...
// 如果使用c++编译器，该函数会使用C风格
MY_INFO_EXPORT FibonacciInterface* CreateFibonacciInstance(FibonacciInterface::FibonacciType type);
