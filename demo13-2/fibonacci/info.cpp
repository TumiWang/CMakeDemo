...
FibonacciItem FibonacciItem::operator+(const FibonacciItem& other) const {
    ...
    // 这个 if 条件是校验
    if (v_ - 1 == other.v_ || (v_ == 1 && other.v_ == 1)) {
        result.v_ = v_ + 1;
        result.InitText();
    }
...
public:
    FibonacciImpl(bool first_is_zero);
...
FibonacciImpl::FibonacciImpl(bool first_is_zero) : mac_cache_index_(1) {
    if (first_is_zero) {
        cache_.insert(cache_.end(), {0, FibonacciItem(0)});
    } else {
        cache_.insert(cache_.end(), {0, FibonacciItem(1)});
    }
...
FibonacciInterface* CreateFibonacciInstance(bool first_is_zero) {
    return new FibonacciImpl(first_is_zero);
}